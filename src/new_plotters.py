...
from src.tools import linked_mapping

def plot_fixed_linked_sdes_images(X, Y, SDEs, n_samples=4, gray=False):
    assert len(X) == len(Y) and len(Y) >= n_samples
    with torch.no_grad():
        T_X = torch.stack([linked_mapping(SDEs, X) for i in range(n_samples)], dim=0)

    imgs = (
        torch.cat((X[None, :], T_X, Y[None, :]), dim=0)
        .detach()
        .cpu()
        .permute(0, 1, 3, 4, 2)
    )
...
def plot_random_linked_sdes_images(XY_sampler, SDEs, n_samples=4, gray=False):
    X, Y = XY_sampler.sample(5)
    X, Y = X.flatten(start_dim=0, end_dim=1), Y.flatten(start_dim=0, end_dim=1)
    return plot_fixed_linked_sdes_images(X, Y, SDEs, n_samples, gray)


def plot_several_fixed_linked_sdes_trajectories(X, Y, SDEs, steps_to_show, times, gray=False):
    n_trajectories = 3
    X = torch.repeat_interleave(X[:4], repeats=n_trajectories, dim=0)
    Y = torch.repeat_interleave(Y[:4], repeats=n_trajectories, dim=0)

    with torch.no_grad():
        trajectory = linked_mapping(SDEs, X, return_trajectory=True)
        trajectory = torch.transpose(trajectory, 0, 1)
        trajectory = torch.stack([trajectory[step] for step in steps_to_show], dim=0)

    imgs = (
        torch.cat((X[None, :], trajectory, Y[None, :]), dim=0)
        .detach()
        .cpu()
        .permute(0, 1, 3, 4, 2)
    )
...
def plot_several_random_linked_sdes_trajectories(
    XY_sampler, SDEs, steps_to_show, times, gray=False
):
    X, Y = XY_sampler.sample(5)
    X, Y = X.flatten(start_dim=0, end_dim=1), Y.flatten(start_dim=0, end_dim=1)

    return plot_several_fixed_linked_sdes_trajectories(X, Y, SDEs, steps_to_show, times, gray)
...
def plot_several_random_sde_trajectories(
    X_sampler, Y_sampler, T, steps_to_show, times, gray=False
):
    X, Y = X_sampler.sample(10), Y_sampler.sample(10)
