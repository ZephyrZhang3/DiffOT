...
def get_euler_maruyama_solution(
    sde,
    x0,
    times,
    create_graph=False,
    ...
    for i, (t, delta_t) in enumerate(zip(times[:-1], delta_times)):
        ...
        x0 = tuple(
            [
                x_elem
                + drift * delta_t
                + torch.sqrt(epsilon * delta_t)
                * torch.randn_like(x_elem, device=x_elem.device)
                for x_elem, drift, epsilon in zip(x0, drifts, epsilons)
            ]
        )
        if not create_graph:
...
class TimeEmbedding(nn.Module):
    def __init__(self, dim, scale):
        ...
        inv_freq = torch.exp(
            ...
            * (-torch.log(torch.tensor(10000)) / dim)
        )
...
class EnotSDE(nn.Module):
    def __init__(
        ...
        self.drift_model = DriftModel(
            dataset_1_channels=dataset_1_channels,
            dataset_2_channels=dataset_2_channels,
            time_dim=time_dim,
            unet_base_factor=unet_base_factor,
            n_steps=n_steps,
            predict_drift_from_new_state=predict_drift_from_new_state,
        )
    ...
    def forward(self, x, create_graph=False, return_trajectory=False):
        ...
        x, norm_square = get_euler_maruyama_solution(
            self,
            x0,
            self.times,
    ...
    def get_drift(self, x, t, **kwargs):
        ...
        x_norm_square_drift = torch.norm(
            torch.flatten(x_coordinate_drift, start_dim=1, end_dim=-1), p=2, dim=-1
        ).square()
    ...
    def set_epsilon(self, epsilon):
        self.register_buffer(
            "epsilon",
            torch.tensor(epsilon, device=next(self.parameters()).device),
            persistent=False,
        )
...
class LitENOT(pl.LightningModule):
    def __init__(
        ...
        **kwargs,
        ...
        self.epsilon_scheduler = lambda step: min(
            epsilon, epsilon * (step / (epsilon_scheduler_last_iter))
        )
        self.inner_iters = self.hparams.inner_iters
        self.norm_square_normalization_constant = (
            self.hparams.norm_square_normalization_constant
        )
    ...
    def training_step(self, batch):
        ...
        loss = (
            (norm_square - self.D(XN)).mean()
            if is_generetor_optimization_step
        ...
        if outer_optimization_step % 10 == 0 and not is_generetor_optimization_step:
            ...
            wandb.log({"Mean norm": torch.sqrt(norm_square).mean().item()}, step=step)
            wandb.log({"D_loss": loss.item()}, step=step)
            #             wandb.log({f'T_loss' : T_loss.item()}, step=step)

            #             wandb.log({f'D_loss' : D_loss.item()}, step=step)
            wandb.log({"D_X1": self.D(X1).mean().item()}, step=step)
            wandb.log({"D_XN": self.D(XN).mean().item()}, step=step)
