...
class MNISTM(data.Dataset):
    ...
    def __init__(self, root, mnist_root=None, train=True, transform=None, target_transform=None, download=False):
        """Init MNIST-M dataset."""
        super(MNISTM, self).__init__()
        self.root = os.path.expanduser(root)
        self.raw_folder = os.path.join(self.root, self.__class__.__name__, "raw")
        self.processed_folder = os.path.join(self.root, self.__class__.__name__, "processed")
        if mnist_root is None:
            mnist_root = os.path.join(self.root, self.__class__.__name__)
